...
description: >
  Drive a fresh or near-fresh app from generated skeleton to a working foundation — auth wired, one core data flow proven end-to-end, clean build — through bounded implement/verify/critique rounds with a real stop condition and a hard round ceiling, cutting the back-and-forth of babysitting early build-out.
argument-hint: "<app dir> [the one core flow to prove, e.g. \"todos crud\"]"
---
...
2. **Gather the brief.** From `$ARGUMENTS` and the repo:
   ...
   - Pin down **the one core flow** to prove out — a single noun with a create+read loop (e.g., "todos," "invoices," "posts"). If `$ARGUMENTS` doesn't name one and it isn't obvious from the scaffold, ask one question before starting; picking the wrong flow wastes a full round.
...
7. **Stop condition and hard ceiling — the anti-`gauntlet-loop` rule.** Unlike `gauntlet-loop`'s intentionally unreachable bar ("it will not finish by its own definition"), this skill is *supposed* to finish:
   ...
   - **Hard ceiling:** a real, finite `max_iterations` (default **5 rounds**, override via `$ARGUMENTS` or ask if unclear) and a turn/token budget per round. Hitting the ceiling without passing is a **stop, not a reason to keep going** — report exactly what's still failing and hand back a concrete gap list for a human decision. Never quietly extend the ceiling mid-run.
