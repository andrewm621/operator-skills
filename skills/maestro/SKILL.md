...
name: maestro
description: >
  Standing cross-provider command deck for the Maestro task board — create, claim, work, route, and message across tasks/providers (Claude, Codex, Cursor, Grok Build), persisting across sessions and terminals via markdown + git so any agent can pick up where another left off, and ask/answer questions along the way.
...
1. **Parse the subcommand** from `$ARGUMENTS` — the first token is the subcommand, everything after it is passed through verbatim (quoted title/text, `--flag value` pairs).
