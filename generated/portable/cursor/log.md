...
Arguments: {the text you type after the command} (optional: entry text, "view", "wrap" to finalize, or a direction like "focus on auth decisions")

## Steps

1. **Parse the intent** — Determine what the user wants from `{the text you type after the command}`:
...
3. **Generate the log entry** — Based on the action:
   ...
   **For auto-generated entries (empty `{the text you type after the command}`):**
   ...
   **For directed entries (`{the text you type after the command}` has text or a focus area):**
...
5. **Wrap-up (when `{the text you type after the command}` is "wrap")** — Finalize the daily log:
