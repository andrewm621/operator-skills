...
Arguments: {the text you type after the command} (optional: topic override or "notion" to also create a Notion page)
...
3. **Determine the topic** — If `{the text you type after the command}` provides a topic, use it. Otherwise, infer from the session:
...
7. **Optionally create Notion page** — If `{the text you type after the command}` includes "notion":
