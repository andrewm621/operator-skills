...
Arguments: {the text you type after the command} (optional: topic slug, "list", "thaw", "thaw <slug>", or "all")

## Instructions

Parse `{the text you type after the command}` to determine the action:
...
If `{the text you type after the command}` is non-empty and isn't a subcommand, use it as the slug.
...
When `{the text you type after the command}` starts with `thaw`:
...
When `{the text you type after the command}` is `all`:
...
When `{the text you type after the command}` is `list`:
