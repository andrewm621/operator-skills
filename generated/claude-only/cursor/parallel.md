...
1. **Parse tasks** — Extract individual tasks from `{the text you type after the command}`:

   **Pipe-separated format:**
   Split `{the text you type after the command}` on `|` (pipe character) and trim whitespace from each segment.
   ...
   **Phase reference format:**
   If `{the text you type after the command}` starts with "phase", parse the phase number and optional task IDs.
   ...
   **Build-check format:**
   If `{the text you type after the command}` starts with "build-check", parse project names.
