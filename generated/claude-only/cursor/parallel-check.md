...
Arguments: {the text you type after the command} (optional: specific projects to check, or "all" for all consumers)
...
1. **Detect what changed and find consumers:**
   ...
   **If in `~/Projects/design-system/`** (most common case):
   ...
   - If `{the text you type after the command}` specifies projects, use those instead of auto-detecting
   - If `{the text you type after the command}` is "all", check every consumer found
